...
const FORMS = [
  ...
  },
  {
    ...
    note: "Create a dedicated DSM user (Control Panel → User & Group) WITHOUT 2-factor auth — the API login can't answer OTP prompts. Storage Manager stats (volumes, disk health) need the administrators group; CPU/RAM works for any user. If the NAS runs on a DSM power schedule, set the offline window below so a sleeping NAS isn't reported as a fault — it will still alert if it fails to wake.",
    fields: [
      ...
      { key: "password", label: "PASSWORD", secret: true },
      { key: "offline_from", label: "SCHEDULED OFF FROM", placeholder: "23:00",
        hint: "optional — 24h local time, matching DSM's power schedule" },
      { key: "offline_to", label: "SCHEDULED OFF UNTIL", placeholder: "07:00",
        hint: "may cross midnight; leave both blank to disable" },
      { key: "offline_grace_min", label: "WAKE GRACE (MIN)", placeholder: "15",
        hint: "boot time allowed after the window before it counts as DOWN" },
    ],
    tls: true,
  },
  {
